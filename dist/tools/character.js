...
export function parseCharacterData(raw) {
    ...
    // ── Template resolver ─────────────────────────────────────────────────────
    const resolveTemplates = (text) => text
    ...
    if (num(weightSpeeds.fly) > 0)
        speedParts.push(`fly ${num(weightSpeeds.fly)} ft.`);
    if (num(weightSpeeds.swim) > 0)
        speedParts.push(`swim ${num(weightSpeeds.swim)} ft.`);
    if (num(weightSpeeds.climb) > 0)
        speedParts.push(`climb ${num(weightSpeeds.climb)} ft.`);
    if (num(weightSpeeds.burrow) > 0)
        speedParts.push(`burrow ${num(weightSpeeds.burrow)} ft.`);
    // ── Initiative ────────────────────────────────────────────────────────────
    ...
    const reactions = ["• Opportunity Attack", ...reactionSpells];
    ...
    for (const c of classes) {
        ...
        spellcastingLines.push(`  ${str(def.name)}: ${statNames[abilityId - 1]}  Spell Attack: ${signed(spellAttack)}  Save DC: ${saveDc}`);
    }
    // ── Spell Slots ───────────────────────────────────────────────────────────
    ...
    if (disguisedFeats.length) {
        lines.push(`OTHER FEATURES (stored as feats in API but NOT player-chosen feats)`, ...disguisedFeats.map(f => `• ${str(obj(f.definition).name)}`), ``);
    }
    lines.push(`CLASS FEATURES`, ...classFeatureLines, ``, `RACIAL TRAITS`, ...racialTraitLines, ``);
    if (bgFeatureName) {
        const descSnippet = bgFeatureDesc
            ? `${bgFeatureDesc}${bgFeatureDesc.length >= 300 ? "…" : ""}` : "";
        lines.push(`BACKGROUND FEATURE`, `  ${bgFeatureName}${descSnippet ? `: ${descSnippet}` : ""}`, ``);
    }
    lines.push(`ACTIONS`, ...(weaponAttacks.length ? weaponAttacks : ["  (none)"]), ``, `BONUS ACTIONS`, ...(bonusActions.length ? bonusActions : ["  (none)"]), ``, `REACTIONS`, ...(reactions.length ? reactions : ["  (none)"]), ``);
    ...
    lines.push(`ATTUNEMENT: ${attuned}/3 slots used`, ``, `CURRENCY: ${currencyLine}`);
    return lines.join("\n");
}
export async function parseCharacter(characterId) {
