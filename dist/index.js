...
// ─── ddb_close_browser ───────────────────────────────────────────────────────
server.tool("ddb_close_browser", "Close the background browser window if one is open. Useful after running ddb_navigate, ddb_interact, or ddb_current_page.", {}, async () => {
    try {
        await closeBrowser();
...
// ─── ddb_get_character ────────────────────────────────────────────────────────
server.tool("ddb_get_character", "Fetch raw character JSON from the D&D Beyond API. WARNING: the response is 300–500 KB of unprocessed data and will consume a large number of tokens. Prefer ddb_parse_character for any play-related question — only use this tool when you explicitly need the raw JSON (e.g. before ddb_download_character, or to inspect fields not surfaced by ddb_parse_character).", {
...
// ─── ddb_navigate ─────────────────────────────────────────────────────────────
server.tool("ddb_navigate", "Navigate to any D&D Beyond URL and return the page's text content. Only dndbeyond.com URLs are allowed. The browser stays open after this call for follow-up ddb_interact or ddb_current_page calls. Call ddb_close_browser when finished.", {
    url: z
        .string()
        .min(1)
...
// ─── ddb_current_page ─────────────────────────────────────────────────────────
server.tool("ddb_current_page", "Return the text content of the currently loaded page in the browser. The browser stays open — call ddb_close_browser when finished.", {}, async () => {
    try {
        const context = await getSharedContext();
